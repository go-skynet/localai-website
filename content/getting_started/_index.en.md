...
For a step by step how to of setting up LoaclAI, Please see our [How to]({{%relref "howtos" %}}) page. 

The here is a easy way to run LocalAI is by using `docker-compose` or with `docker` (to build locally, see the [build section]({{%relref "build" %}})), the following example uses `docker-compose`:
